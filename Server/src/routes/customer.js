...
router.get('/:customer1/:custemail/:custpassword', (req, res) => {
    ...
    mysqlConnection.query('select * from customer where custemail = ? and custpassword =? ;', [email, password], (error, rows, fields) => {
        if (!error) {
            if(rows.length==0){
                res.send("NO entries");
...
router.post('/:customer', (req, res) => {
    ...
    console.log(phone);
    console.log(pass);
    mysqlConnection.query('insert into customer values(?,?,?,?,?);', [id, name, phone, email, pass], (error, rows, fields) => {
        if (!error) {
